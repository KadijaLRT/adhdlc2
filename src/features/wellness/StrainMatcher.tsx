      ...
      <View className="w-full max-w-md self-center gap-4">
        <View className="bg-slate-900 rounded-2xl p-5">
          <View className="flex-row items-center justify-between mb-1">
            <Text className="text-slate-100 text-base font-semibold">Strain Explorer</Text>
          ...
          <>
              ...
              {(CANNABIS_EFFECTS || []).map((effect) => {
                ...
                return (
                  <Pressable key={effect} onPress={() => setSelectedEffect(effect)}
                    className={isActive ? 'bg-emerald-400/10 border-2 border-emerald-400 rounded-full py-2 px-4' : 'bg-slate-800 border-2 border-transparent rounded-full py-2 px-4'}>
                    <Text className={isActive ? 'text-emerald-300' : 'text-slate-300'}>{effect}</Text>
                  </Pressable>
                );
              })}
            </View>
            {selectedEffect && (
              <View className="bg-slate-900 rounded-2xl p-4 mt-2">
                {(matches || []).length
                  ? (matches || []).map((strain) => <Text key={strain} className="text-slate-200 mb-1">{strain}</Text>)
