...
export default function DayRhythmCard() {
  ...
  return (
    <View className="bg-slate-900 rounded-2xl p-5 w-full">
      <Text className="text-slate-100 text-base font-semibold mb-1">How&apos;s your energy today?</Text>
      <Text className="text-slate-400 text-xs mb-4">This helps shape what we show you today. You can change it anytime.</Text>
      <View className="flex-row gap-2">
        {(ENERGY_OPTIONS || []).map((option) => {
          const isActive = energyLevel === option.level;
          return (
            <Pressable key={option.level} onPress={() => logEnergyForToday(option.level)}
              className={isActive ? 'flex-1 bg-indigo-600/20 border-2 border-indigo-400 rounded-xl py-3 items-center' : 'flex-1 bg-slate-800 border-2 border-transparent rounded-xl py-3 items-center active:border-slate-600'}>
              <Text className="text-xl mb-1">{option.emoji}</Text>
              <Text className={isActive ? 'text-indigo-200 text-sm font-medium' : 'text-slate-300 text-sm font-medium'}>{option.label}</Text>
            </Pressable>
          );
        })}
      </View>
