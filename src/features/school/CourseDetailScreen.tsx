    ...
    <ScrollView className="flex-1" contentContainerStyle={{ padding: 20 }}>
      <View className="w-full max-w-md self-center">
        ...
        <View className="bg-slate-900 rounded-2xl p-4 mb-4">
          <Text className="text-slate-300 text-sm font-medium mb-2">Grade</Text>
        ...
        <View className="bg-slate-900 rounded-2xl p-4 mb-4">
          <Text className="text-slate-300 text-sm font-medium mb-2">Notes & flashcards</Text>
          <TextInput
            value={notesText}
            ...
            placeholderTextColor="#64748b"
            multiline
            className="bg-slate-800 text-slate-100 rounded-xl p-3 min-h-[80px] mb-2"
          />
          ...
          {flashcards?.cards?.length ? (
            <View className="gap-2">
              {flashcards.cards.map((card, i) => (
                <View key={i} className="bg-slate-800 rounded-lg p-3">
                  <Text className="text-slate-100 text-sm mb-1">{card.front}</Text>
        ...
        <View className="bg-slate-900 rounded-2xl p-4 mb-4">
          <Text className="text-slate-300 text-sm font-medium mb-2">New assignment</Text>
          <TextInput
            value={newTitle}
            onChangeText={setNewTitle}
            placeholder="Research paper, Chapter 6 reading..."
            placeholderTextColor="#64748b"
            className="bg-slate-800 text-slate-100 rounded-xl px-4 py-3 mb-2"
          />
          <View className="flex-row gap-2">
            <TextInput
              value={newDueDate}
        ...
        <View className="gap-2">
          ...
          {courseAssignments.map((a) => (
            <Pressable key={a.id} onPress={() => router?.push?.(`/school/assignment/${a.id}`)} className="bg-slate-900 rounded-xl p-4 flex-row items-center justify-between">
              <Text className={a.isComplete ? 'text-slate-500 line-through flex-1' : 'text-slate-100 flex-1'}>{a.title}</Text>
