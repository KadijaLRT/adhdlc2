          ...
          <View key={month} className="mb-5">
            ...
            <View className="gap-2">
              {items.map((a) => {
                ...
                return (
                  <View key={a.id} className="bg-slate-900 rounded-xl p-3 flex-row items-center gap-3">
                    <View style={{ width: 4, height: '100%', minHeight: 32, backgroundColor: courseColor(a.courseId), borderRadius: 2 }} />
                    <View className="flex-1">
                      <Text className={a.isComplete ? 'text-slate-500 line-through text-sm' : 'text-slate-100 text-sm'}>{a.title}</Text>
