...
export default function PersonalizeFitnessCard() {
  ...
  return (
    <View className="bg-slate-900 rounded-2xl p-5 mx-4 mb-4">
      <Text className="text-slate-100 text-base font-semibold mb-1">Want a workout list matched to you?</Text>
      <Text className="text-slate-400 text-xs mb-4">
        We'll only show exercises you can actually do. Change anytime.
      </Text>

      <Text className="text-slate-300 text-xs font-medium mb-2">What do you have access to?</Text>
        ...
        {(EQUIPMENT_OPTIONS || []).map((item) => {
          ...
          return (
            <Pressable
              ...
              className={isActive ? 'bg-emerald-400/10 border-2 border-emerald-400 rounded-full py-2 px-3' : 'bg-slate-800 border-2 border-transparent rounded-full py-2 px-3'}
            >
              <Text className={isActive ? 'text-emerald-300 text-xs capitalize' : 'text-slate-300 text-xs capitalize'}>{item.replace('_', ' ')}</Text>
            </Pressable>
          );
        })}
      </View>

      <Text className="text-slate-300 text-xs font-medium mb-2">Main goal</Text>
